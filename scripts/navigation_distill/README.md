...
| 10. XY re-score | `python3 $T/rescore_xy.py --packet $NAV_PACKET --output <new dir>` (numpy only, read-only on the packet) | `rescore.csv` (one row per unique episode: legacy 3-D flag and its exact reproduction, XY-goal flags, final/min goal error, time to goal, path ratio, height deviation), `rescore_summary.{md,json}` per panel |
