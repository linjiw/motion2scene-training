...
| 10. Goal/map-use ablation (roadmap 0.4) | `bash $T/run_goal_ablation.sh [--dry] <nav.pt> S` (19 registered feasible tasks; `run_stage.sh nav ... --goal-rotation-deg 90` and `... --zero-obstacles`), then `$PY $T/analyze_goal_ablation.py --panel <stage> --baseline <unablated stage>` | the actor sees a goal rotated about the start, or an empty map; scoring and physics keep the original task, and `task-result.json` gains a `navigation_ablation` block with final XY distances to the original goal, the rotated goal and the reference endpoint |
