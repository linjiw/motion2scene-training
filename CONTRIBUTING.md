...
Install the base test dependencies with `pip install -e '.[test]'` and run `python -m pytest -q tests`. Native tests require the documented Isaac/SONIC dependencies. Format new package code with Black and check it with Ruff. When editing vendored code, record the original snapshot hash, current hash and reason in `manifests/vendor-amendments.json`.
