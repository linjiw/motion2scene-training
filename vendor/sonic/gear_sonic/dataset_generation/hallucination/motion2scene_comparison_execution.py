"""Explicit encounter command with direct, pre-command learning-state capture.

Derived from the frozen action executor; previous runtime and records stay immutable.
"""
...
class ComparisonCommand(EvaluationBankCommand):
...
class ComparisonEnvCfg(EvaluationBankEnvCfg):
    def __init__(self, config, **kwargs):
        super().__init__(config, **kwargs)
        self.commands.motion.class_type = ComparisonCommand
...
class ComparisonRecorder(EvaluationBankRecorder):
    def close_writers(self):
        ...
        data["rule_inputs"] = (
            "explicit binary encounter action and chosen decision tick; scene-independent phase/jump guard"
        )
        data["limitations"] = (
...
@configclass
class ComparisonRecorderCfg(EvaluationBankRecorderCfg):
    class_type = ComparisonRecorder
