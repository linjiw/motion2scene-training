"""Same frozen transition path with a common learned/scripted/privileged readout adapter."""
...
from .motion2scene_icra_readout import load_readout, readout
...
class LearnedCommand(EvaluationBankCommand):
    ...
    def _update_command(self):
        ...
        if not self.decision_issued and abs(time_s - self.cfg.decision_time_s) < 1e-8:
            ...
            decision = readout(
                self.learned_readout, self.decision_capture["features"], packet=delivered
            )
...
class LearnedRecorder(EvaluationBankRecorder):
    def close_writers(self):
        ...
        data["rule_inputs"] = {
            "kind": command.learned_readout["kind"],
            "shared_guard": "phase/jump legality only",
            "learned_inputs": "214 directly captured causal features",
            "scripted_inputs": "actual packet occupied bit",
            "privileged_inputs": "registered achieved-geometry forecast for known scene",
        }
        data["limitations"] = (
