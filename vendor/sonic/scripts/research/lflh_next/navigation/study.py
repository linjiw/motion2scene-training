...
OUT = ROOT.parent / "research-data/m2s-kimodo-navigation-100-20260911"
...
def main():
    ...
    write(
        ...
        dict(
            ...
            prior_access="Previous AMASS and seven-schedule outcomes accessible; Kimodo aggregate reports inspected; no new Kimodo placement outcomes yet",
            scope="100 generated references fit, 20 prompt-component-separated development references; unchanged 225 candidate shapes/positions",
    ...
    encoded = torch.cat([specs[:, :3] / 3, nn.functional.one_hot(specs[:, 3].long(), 3)], -1)
    torch.save(
        dict(lower=lower, inner_upper=upper, specs=specs, features=X, mean=mean, scale=scale),
        OUT / "labels.pt",
    )
