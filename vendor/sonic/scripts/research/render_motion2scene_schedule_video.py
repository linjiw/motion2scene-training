#!/usr/bin/env python3
"""Visualize selected measured seven-schedule branches without dynamics integration."""
...
def render(result_path, out, options, preview=False):
    ...
    if scene["split"] != "development" or len(scene["beams"]) != 1:
        raise ValueError("one explicit development beam required")
    indexed = {row["forced_option_id"]: row for row in result["rows"]}
    if len(options) != 3 or len(set(options)) != 3 or not set(options).issubset(indexed):
        raise ValueError("three distinct recorded forced schedules required")
    rows, models, traces, sources = [indexed[key] for key in options], [], [], []
    beam = scene["beams"][0]
    ref = manifest["request"]
    request = json.loads(checked(Path(ref["path"]), ref["sha256"]).read_text())
    definitions = {row["option_id"]: row for row in request["options"]}
    names = {"short": "Short", "sustained": "Sustained", "prior_splice": "Prior derivative"}
    labels = [
        (
            "Walk"
            if key == "neutral"
            else f"{names[definitions[key]['reference_id']]} | {definitions[key]['entry_tick'] / 50:.2f} s"
        )
        for key in options
    ]
    for row in rows:
    ...
    try:
        ...
        for k in ([136] if preview else list(range(0, 298, 2))):
            ...
            draw.text(
                (20, 14),
                "Motion2Scene | Executed prior alternatives",
                font=fonts[29],
                fill="#edf4f7",
            )
            draw.text(
    ...
    write_new(
        ...
        dict(
            ...
            scene=scene,
            records=sources,
            video=artifact(out / "schedule_execution.mp4") if not preview else None,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--options", nargs=3, required=True)
    parser.add_argument("--preview", action="store_true")
    args = parser.parse_args()
    render(args.result, args.out, args.options, args.preview)
