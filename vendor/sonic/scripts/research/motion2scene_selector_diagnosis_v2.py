...
PROTOCOL = ROOT / "docs/motion2scene/SELECTOR_DIAGNOSIS_NUMERICAL_REVISION_V2.md"
GROUPS = {
    "rays": (0, 144),
...
def register(out):
    ...
    refs += [
        artifact(ROOT / "docs/motion2scene/SELECTOR_BREAKPOINT_STUDY_V1.md"),
        artifact(DATA / "m2s-selector-input-diagnostic-v1/registration.json"),
        artifact(DATA / "m2s-selector-input-diagnostic-v1/batch_readout_failure.json"),
        artifact(DATA / "m2s-selector-input-diagnostic-v1/analysis_failure.log"),
    ]
...
def network_details(policy, x):
    if len(x) > 1:
        values = [network_details(policy, row[None]) for row in x]
        return tuple(np.concatenate([v[k] for v in values]) for k in range(5))
