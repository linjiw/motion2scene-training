...
PROTOCOL = ROOT / "docs/motion2scene/LINEAR_SELECTOR_EXECUTION_V2.md"
...
def prepare(out):
    ...
    for control in p0["linear_controls"]:
        ...
        assert max(errors) <= 2e-6 and actions
        equivalence.append(
            {
                "arm": control["arm"],
                "inputs": len(inputs),
    ...
    refs = [
        artifact(PROTOCOL),
        artifact(DATA / "m2s-shared-linear-control-v1/prepare_failure.json"),
        artifact(DATA / "m2s-shared-linear-control-v1/motion2scene_linear_execution.py"),
        artifact(DATA / "m2s-shared-linear-control-v1/LINEAR_SELECTOR_EXECUTION_V1.md"),
    ...
    m.update(
        experiment="M2S-shared-linear-control-v2",
        cells=cells,
