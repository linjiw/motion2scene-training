#!/usr/bin/env python3
"""Replay a fixed first-wave subset: four learners, all three heights and both physics seeds."""
...
def render(out, output):
    ...
    ref = prepared["master"]
    master = json.loads(checked(Path(ref["path"]), ref["sha256"]).read_text())
    ...
    with imageio.get_writer(
        ...
        for block_index in range(6):
            block = master["blocks"][block_index]
            assert block["physics_seed"] in (8511, 8512)
            folder = Path(block["directory"])
            admission = json.loads((folder / "admission.json").read_text())
            assert admission["admitted"]
            sources.append(artifact(folder / "admission.json"))
            ...
            for arm in ARMS:
                row = next(
                    r for r in result["rows"] if r["arm"] == arm and r["optimizer_seed"] == 8501
                )
                ref = row["trajectory"]
            ...
            try:
                for i in range(100):
                    ...
                    draw.text(
                        (12, 765),
                        f"Reserved station 0.35 | beam underside {beam['underside_m']:.2f} m "
                        f"| elapsed {k/50:.2f} s",
                        font=small,
                        fill="#eaf3f5",
                    )
                    draw.text(
                        (12, 788),
                        f"All four seed-8501 learners; physics seed {block['physics_seed']}; source 41002.",
                        font=small,
                        fill="#eaf3f5",
                    )
                    draw.text(
        ...
        json.dumps(
            {
                ...
                "fps": 25,
                "selection": (
                    "optimizer seed 8501 and both physics seeds 8511/8512, "
                    "all three first-station heights and all four arms"
                ),
                "scope": (
                    "24 of 120 first-wave executions; full capture intervals retained, "
