#!/usr/bin/env python3
"""Sample a saved explicit-margin model and reject failures under its finite pose audit.
...
from motion2scene_uncertainty_learning import numpy_perturbed, verdict
import numpy as np
import torch
...
def main():
    ...
    parser.add_argument(
        "--arm",
        choices=("margin_full", "margin_no_kl", "constraints_full", "constraints_no_kl"),
        default="margin_full",
    )
    parser.add_argument("--model-seed", type=int, default=8121)
    parser.add_argument("--sample-seed", type=int, default=9521)
    ...
    matches = [ref for ref in completion["cells"] if Path(ref["path"]) == cell / "result.json"]
    if len(matches) != 1:
        raise ValueError("checkpoint cell not pinned in completion record")
    result = json.loads(checked(Path(matches[0]["path"]), matches[0]["sha256"]).read_text())
