...
def main():
    ...
    out = data / "m2s-learning-input-audit-v3"
    ...
    refs = [
        ...
        artifact(data / "m2s-learning-input-audit-v2/result.json"),
    ]
    ...
    write_new(
        out / "registration.json",
        {
            ...
            "predictions": {
                ...
                "state_binding": (
                    "packet upper-ray origin matches recorded root at same recorder index within 1e-6 m; "
                    "audit all available 199 aligned frames per cell"
                ),
            },
            "scope": "CPU measurement contract, not policy fitting; all 199 packet origins checked, including terminal packet",
        },
    )
    rows = []
    for row in result["rows"]:
        ...
        for i, o in enumerate(s["observations"]):
            ...
            if i < len(p["root_pos_w"]):
                root = np.asarray(p["root_pos_w"])[i]
                w, x, y, z = np.asarray(p["root_quat_w"])[i]
                ...
                phase_errors.append(abs(float(p["motion_time_s"][i]) - o["time_s"]))
        a = np.array(ages)
        rows.append(
            {
                "cell_id": row["cell_id"],
    ...
    write_new(
        out / "result.json",
        {
            ...
            "scope": (
                ...
                "same recorder index is the same observed robot state, "
