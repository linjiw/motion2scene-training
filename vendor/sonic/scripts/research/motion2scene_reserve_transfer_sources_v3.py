...
from motion2scene_fresh_sources import DATA, SIBLING, seed_mentions
from motion2scene_timing_diagnostic import ROOT, artifact, write_new

IDS = list(range(9470001, 9470009))
...
def scan(raw, ids):
    ...
    if not re.search(pattern, raw) and b"seed_base" not in raw and b"\\" not in raw:
        return set()
    return seed_mentions(json.loads(raw), ids)
...
def main():
    ...
    out = DATA / "m2s-final-transfer-reservation-v3"
    ...
    write_new(
        out / "inventory.json",
        {
            ...
            "scope": "local JSON files; raw negative proof before parsing, original integer/range scan on all possible hits; not a whole-file JSON-validity audit",
        },
    )
    ...
    write_new(
        out / "reservation.json",
        {
            ...
            "scope": "No integer/range mentions under the declared parser in inventoried local JSON; negative byte checks do not certify JSON validity; unrecorded or external history is outside this claim",
