# Shared linear-control execution v2
...
2e-6 and exact action/refusal. Do not change the threshold to achieve equivalence.
...
## Preserved numerical preflight failure

v1 stopped before manifest registration or physics: the mathematically equivalent
linear embedding differed from its vector-matrix evaluation by at most 1.52e-6 in
probability (analytic 1.13e-6, uniform 1.52e-6), exceeding its 1e-6 tolerance. All
158 action decisions per arm agreed exactly. Preserve the v1 script, protocol,
partial checkpoint and measured failures. v2 uses a declared 2e-6 representation
comparison tolerance before any rollout; it changes no fitted parameters, threshold
or runtime. Actual deployed-checkpoint readout auditing retains its 1e-6 tolerance.
This roundoff issue does not explain the original zero-d040 behavior.
