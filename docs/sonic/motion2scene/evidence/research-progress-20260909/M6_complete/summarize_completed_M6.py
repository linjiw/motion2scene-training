"""Summarize complete M6 bindings without repeating the raw-record audit."""
...
OUT = OUTDIR / "all_M6_bound_response_tables.json"
...
def main():
    ...
    previous_ref = artifact(OUTDIR / "all_M5_bound_response_tables.json")
    assert (
        previous_ref["sha256"]
        == "sha256:c7f6d13d953cc1927ca3a72d6d9b510a77a5e89a43c06be47d38b6dd784f83ef"
    )
    ...
    completions = sorted(RUNROOT.glob("*/controller/complete_006.json"))
    ...
    for path in completions:
        ...
        assert c["completed_through_round"] == 6
        assert c["reserved_evaluation_started"] is False
        assert len(groups) == 7 and groups[:-1] == prior_groups
        ...
        pre_ref = artifact(root / "round_006/preupdate.json")
        pre = read_bound(pre_ref)
        release = read_bound(artifact(root / "round_006/student_complete.json"))
        ...
        reports.append(
            dict(
                ...
                M5_bound_teacher_prefix_unchanged=True,
                tasks=tasks,
                responses=response,
    ...
    for seed in sorted({r["seed"] for r in reports}):
        ...
        for a, b in zip(run_a["rounds"][:7], run_b["rounds"][:7], strict=True):
            assert all(
        ...
        pairs.append(
            dict(
                ...
                candidate_order_through_M6_unchanged=True,
    ...
    report = dict(
        ...
        plan=plan_ref,
        prior_M5_bound_readout=previous_ref,
        corpora=reports,
        ...
        interpretation=(
            ...
            "and explicit teacher unknowns. Student outcomes belong to generating M5 policies on each "
            "arm's acquired scene, not the new M6 fits or a common evaluation set. Earlier failures "
