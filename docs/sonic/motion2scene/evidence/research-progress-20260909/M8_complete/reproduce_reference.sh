...
runs=[next(r for r in plan['runs'] if r['run_id']==f'seed{seed}_{arm}') for arm in ['reference_contrast']]
reports=[]
for run in runs:
...
report=dict(schema='motion2scene_completed_acquired_corpora_readout_v1',utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),plan=plan_ref,budget=8,physics_seed=seed,corpora=reports,new_physics_from_readout=0,new_fits_from_readout=0,verification_scope='Hash-bound completed collection/training receipts, actual policy/scene/branch identities and unchanged previous teacher prefix. Uses original collection scoring; no second raw-record audit.',interpretation='One reference-arm acquired scene. The student outcome belongs to its generating M7 policy, not the new M8 fit or a common-set/held-out evaluation. All earlier bank-unsolvable tasks and the reconciled student unknown remain in the acquisition history and measured cost. Finite-bank coverage is not perceptual realizability. Passage time is crossing plus stabilization, not full adaptation/recovery completion. No queue, teacher, learner, scoring or reserved assignment changes.')
out=o/f'seed{seed}_reference_M8_bound_readout.json';assert not out.exists();out.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
