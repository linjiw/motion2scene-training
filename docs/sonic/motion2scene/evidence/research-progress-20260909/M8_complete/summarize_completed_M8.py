"""Summarize complete M8 bindings without repeating the raw-record audit."""
...
OUT = OUTDIR / "all_M8_bound_response_tables.json"
...
def main():
    ...
    previous_ref = artifact(OUTDIR / "all_M7_bound_response_tables.json")
    assert (
        previous_ref["sha256"]
        == "sha256:c98a5ac17b294502902131aa3dc2079852676071a1ffd5a907776cbca7136096"
    )
    ...
    completions = sorted(RUNROOT.glob("*/controller/complete_008.json"))
    ...
    for path in completions:
        ...
        assert c["completed_through_round"] == 8
        assert c["reserved_evaluation_started"] is False
        assert len(groups) == 9 and groups[:-1] == prior_groups
        ...
        pre_ref = artifact(root / "round_008/preupdate.json")
        pre = read_bound(pre_ref)
        release = read_bound(artifact(root / "round_008/student_complete.json"))
        ...
        reports.append(
            dict(
                ...
                M7_bound_teacher_prefix_unchanged=True,
                tasks=tasks,
                responses=response,
    ...
    for seed in sorted({r["seed"] for r in reports}):
        ...
        for a, b in zip(run_a["rounds"][:9], run_b["rounds"][:9], strict=True):
            assert all(
        ...
        pairs.append(
            dict(
                ...
                candidate_order_through_M8_unchanged=True,
    ...
    report = dict(
        ...
        plan=plan_ref,
        prior_M7_bound_readout=previous_ref,
        corpora=reports,
        ...
        interpretation=(
            ...
            "and explicit teacher unknowns. Student outcomes belong to generating M7 policies on each "
            "arm's acquired scene, not the new M8 fits or a common evaluation set. Earlier failures "
            ...
            "This acquisition-only readout contains no M8 common-set performance; "
            "consult the separate frozen development panel."
        ),
    )
