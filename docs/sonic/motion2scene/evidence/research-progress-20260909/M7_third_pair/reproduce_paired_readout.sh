...
for a,b in zip(runs[0]['rounds'][:8],runs[1]['rounds'][:8],strict=True):assert all(a[k]==b[k] for k in ['index','candidate_id','scene','branch_order'])
reports=[]
for run in runs:
...
a,b=reports
report=dict(schema='motion2scene_paired_completed_acquisition_readout_v1',utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),plan=plan_ref,budget=7,candidate_id=runs[0]['rounds'][7]['candidate_id'],scene=runs[0]['rounds'][7]['scene'],physics_seed=runs[0]['seed'],identical_candidate_scene_and_branch_order_through_M7=True,teacher_tables_identical=a['teacher_outcomes']==b['teacher_outcomes'],teacher_table_disagreements=[k for k in a['teacher_outcomes'] if a['teacher_outcomes'][k]!=b['teacher_outcomes'][k]],corpora=reports,matched_student_passage_difference=int(a['student_task_outcome']=='pass')-int(b['student_task_outcome']=='pass'),matched_student_time_difference_s=(a['student_passage_time_s']-b['student_passage_time_s'] if a['student_task_outcome']==b['student_task_outcome']=='pass' else None),new_physics_from_readout=0,new_fits_from_readout=0,verification_scope='Hash-bound completed collection/training receipts, actual policy/scene/branch identities and unchanged previous teacher prefixes. Uses original collection scoring; no second raw-record audit.',interpretation='Two actual generating M6 student executions on one shared acquired scene/seed. This is not evaluation of the newly fitted M7 policies, a common-set comparison, or a held-out result. Passage time is crossing plus stabilization, not full adaptation/recovery completion. Historical physical outcomes stay attached to their generating policies; this descriptive paired outcome does not establish population equality.')
out=o/f'seed{seed}_paired_M7_bound_readout.json';assert not out.exists();out.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
