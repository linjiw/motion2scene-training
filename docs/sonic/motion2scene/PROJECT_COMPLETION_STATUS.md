...
> **September 11 audit supersedes execution and manuscript status below.** Acquisition has 288 scored receipts; validation has 171 scored receipts, one interrupted launch without terminal/raw receipts, and 28 unrun assignments. Execution is BLOCKED under the frozen no-automatic-retry rule. The current anonymous mechanism draft is `submission/traversal_method_v2.tex`; Phase 3 remains closed. See [complete audit and decision log](audit/20260911-pilot-boundary/README.md). The remainder of this document is a historical record.
