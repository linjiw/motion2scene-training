...
- **Shared GPU:** `launch.py` only requires 2 GiB of free memory. Measured on an RTX 5090, a 128-environment trainer used about 3.7 GiB and an 8192-environment trainer peaked at 16.9 GiB (8.0 GiB of it PyTorch allocations), and throughput drops when other jobs saturate the device, which counts against the 48 h wall cap. `launch.py` sets `CUDA_VISIBLE_DEVICES=0` for the trainer, so it always uses physical GPU 0.
